...
if (@$_POST['approvetxfilter'] || @$_POST['disapprovetxfilter']) {
	if (
		no_displayed_error_result($approvetxid, multichain(
			'approvefrom',
			$_POST['approvefrom'],
			$_GET['txfilter'],
			@$_POST['approvetxfilter'] ? true : false
		))
	)
...
if (@$_GET['txfilter']) {
	if (
		no_displayed_error_result($listtxfilters, multichain(
			'listtxfilters',
			$_GET['txfilter'],
			true
		))
...
if (@$_GET['streamfilter']) {
	if (
		no_displayed_error_result($liststreamfilters, multichain(
			'liststreamfilters',
			$_GET['streamfilter'],
			true
		))
	...
	<div class="col-sm-12">
		...
		if (@$_GET['txfilter']) {
			...
			<form class="form-horizontal" method="post"
				action="./?chain=<?php echo html($_GET['chain']) ?>&page=<?php echo html($_GET['page']) ?>&txfilter=<?php echo html($_GET['txfilter']) ?>">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<div class="form-control-static" style="height:16em; overflow:scroll; white-space:pre;">
							<code><?php echo html($filtercode) ?></code>
						</div>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-9">
						<input class="btn btn-default" type="submit" name="approvetxfilter"
							value="Approve Transaction Filter">
						&nbsp;
						<input class="btn btn-default" type="submit" name="disapprovetxfilter"
							value="Disapprove Transaction Filter">
					</div>
				</div>
			</form>
		...
		if (@$_GET['streamfilter']) {
			...
			<form class="form-horizontal" method="post"
				action="./?chain=<?php echo html($_GET['chain']) ?>&page=<?php echo html($_GET['page']) ?>&streamfilter=<?php echo html($_GET['streamfilter']) ?>">
				<div class="form-group">
					<label class="col-sm-2 control-label">Filter name</label>
					<div class="col-sm-9">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-9">
						<input class="btn btn-default" type="submit" name="approvestreamfilter"
							value="Approve Filter for this Stream">
						&nbsp;
						<input class="btn btn-default" type="submit" name="disapprovestreamfilter"
							value="Disapprove Filter for this Stream">
					</div>
				</div>
			</form>
